...
use reflow_network::{
    ...
    multi_graph::{
        GraphComposer, GraphComposition, GraphLoader, GraphMetadata, GraphNamespaceManager,
...
impl NodeGraphLoader {
    ...
    fn load_from_json(mut cx: FunctionContext) -> JsResult<JsValue> {
        ...
        match result {
            Ok(graph_export) => match serde_json::to_value(&graph_export) {
    ...
    fn load_multiple(mut cx: FunctionContext) -> JsResult<JsValue> {
        ...
        for i in 0..sources_array.len(&mut cx) {
            ...
            let source_type = source_type_val
                .downcast::<JsString, _>(&mut cx)
                .expect("Expecting to cast JS Value to String")
                .value(&mut cx);
            ...
            Ok(graphs) => {
                ...
                for (i, graph) in graphs.iter().enumerate() {
                    match serde_json::to_value(graph) {
                        ...
                        Err(e) => {
                            return cx
                                .throw_error(format!("Failed to serialize graph {}: {}", i, e))
                        }
                    }
                }
                Ok(js_array.upcast())
...
impl NodeGraphNormalizer {
    ...
    fn normalize(mut cx: FunctionContext) -> JsResult<JsValue> {
        ...
        match this.inner.normalize(&mut graph_export) {
            Ok(()) => match serde_json::to_value(&graph_export) {

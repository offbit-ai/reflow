...
use reflow_network::{connector::{ConnectionPoint, Connector, InitialPacket}, graph::{types::GraphExport, Graph}, message::Message, network::{Network, NetworkConfig}};
use crate::{neon_bindings::{js_to_json_value, json_value_to_js, JavaScriptActor, JavaScriptActorState}, runtime};
...
impl NodeNetwork {
    ...
    fn register_actor(mut cx: FunctionContext) -> JsResult<JsUndefined> {
        ...
        let js_actor_bridge = super::JavaScriptActor::new(
            js_actor,
            config,
            inports,
            outports,
    ...
    fn add_connection(mut cx: FunctionContext) -> JsResult<JsUndefined> {
        ...
        if let (Ok(from_obj), Ok(to_obj)) = (
            ...
            let from_actor_str = from_actor.downcast::<JsString, _>(&mut cx).expect("Expected string").value(&mut cx);
            let from_port_str = from_port.downcast::<JsString, _>(&mut cx).expect("Expected string").value(&mut cx);
            let to_actor_str = to_actor.downcast::<JsString, _>(&mut cx).expect("Expected string").value(&mut cx);
            let to_port_str = to_port.downcast::<JsString, _>(&mut cx).expect("Expected string").value(&mut cx);
    ...
    fn add_initial(mut cx: FunctionContext) -> JsResult<JsUndefined> {
        ...
        if let Ok(to_obj) = to_js.downcast::<JsObject, _>(&mut cx) {
            ...
            let actor_str = actor.downcast::<JsString, _>(&mut cx).expect("Expected string").value(&mut cx);
            let port_str = port.downcast::<JsString, _>(&mut cx).expect("Expected string").value(&mut cx);
...
impl NodeGraphNetwork {
    ...
    fn register_actor(mut cx: FunctionContext) -> JsResult<JsUndefined> {
        ...
        // Create JavaScript actor bridge
        let js_actor = Arc::new(RwLock::new(actor_js.root(&mut cx)));
        let js_actor_bridge = JavaScriptActor::new(
            js_actor,
            config,
            inports,
            outports,
        ...
        // Register with the network
        {
            let mut network = this.network.lock().map_err(|e| -> JsResult<JsUndefined>  {
                cx.throw_error(format!("Failed to lock network: {}", e))
            }).unwrap();
...
fn extract_ports(cx: &mut FunctionContext, actor: &JsObject, port_type: &str) -> NeonResult<Vec<String>> {
...
fn extract_config(cx: &mut FunctionContext, actor: &JsObject) -> NeonResult<HashMap<String, JsonValue>> {
    let config_val: Handle<JsValue> = actor.get(cx, "config").unwrap_or_else(|_| cx.undefined().upcast());

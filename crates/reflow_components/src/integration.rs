...
pub async fn mysql_pool_actor(context: ActorContext) -> Result<HashMap<String, Message>, Error> {
    let mut result = HashMap::new();
...
pub async fn mongodb_pool_actor(context: ActorContext) -> Result<HashMap<String, Message>, Error> {
    let mut result = HashMap::new();
...
pub async fn mongo_collection_actor(
    ...
    let mock_documents = match operation {
        ...
        "findOne" => vec![json!({"_id": "doc1", "name": "Sample Document", "status": "active"})],
        "aggregate" => vec![
    ...
    result.insert(
        "count_out".to_string(),
        Message::integer(mock_documents.len() as i64),
    );

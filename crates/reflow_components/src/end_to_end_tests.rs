...
mod tests {
    ...
    use super::super::{get_actor_for_template, Actor};
    use crate::registry;
    ...
    async fn test_zeal_node_structure() {
        ...
        assert_eq!(
            property_values.get("retryCount").unwrap().as_u64().unwrap(),
            3
        );
    }
    ...
    async fn test_registry_actor_mapping() {
        ...
        assert!(
            mongo_collection_actor.is_some(),
            "Should find actor for tpl_mongo_get_collection"
        );
    ...
    async fn test_http_node_to_actor_execution() {
        ...
        assert!(
            output.contains_key("response_out"),
            "Should have response_out"
        );
    ...
    async fn test_postgresql_node_to_actor_execution() {
        ...
        assert!(
            output.contains_key("pool_out"),
            "PostgreSQL actor should output pool_out"
        );
        assert!(
            output.contains_key("status_out"),
            "PostgreSQL actor should output status_out"
        );
        ...
        if let Some(pool_message) = output.get("pool_out") {
            match pool_message {
                reflow_actor::message::Message::String(pool_id) => {
                    assert!(
                        pool_id.starts_with("pg_pool_"),
                        "Pool ID should start with 'pg_pool_'"
                    );
                }
    ...
    async fn test_template_mapping_completeness() {
        ...
        assert!(
            mapping.contains_key("tpl_http_request"),
            "HTTP request template should be mapped"
        );
        assert!(
            mapping.contains_key("tpl_postgresql"),
            "PostgreSQL template should be mapped"
        );
        assert!(
            mapping.contains_key("tpl_mysql"),
            "MySQL template should be mapped"
        );
        assert!(
            mapping.contains_key("tpl_mongodb"),
            "MongoDB template should be mapped"
        );

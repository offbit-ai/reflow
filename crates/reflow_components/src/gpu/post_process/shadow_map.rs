...
pub async fn shadow_map_actor(ctx: ActorContext) -> Result<HashMap<String, Message>, Error> {
    ...
    let resolution = config.get("resolution").and_then(|v| v.as_u64()).unwrap_or(1024) as u32;
    let bias = config.get("bias").and_then(|v| v.as_f64()).unwrap_or(0.005) as f32;
    let ortho_size = config.get("orthoSize").and_then(|v| v.as_f64()).unwrap_or(500.0) as f32;
    ...
    let light_data = cache.get("lights_b64").and_then(|v| v.as_str()).and_then(|s| {
        use base64::Engine;
        base64::engine::general_purpose::STANDARD.decode(s).ok()
    });
    let light_count = cache.get("light_count").and_then(|v| v.as_u64()).unwrap_or(0) as usize;
...
fn build_light_vp(dir: [f32; 3], half_size: f32, near: f32, far: f32) -> [[f32; 4]; 4] {
    ...
    let eye = [-fwd[0] * far * 0.5, -fwd[1] * far * 0.5, -fwd[2] * far * 0.5];

    let up = if fwd[1].abs() > 0.99 { [0.0f32, 0.0, 1.0] } else { [0.0, 1.0, 0.0] };
...
fn render_depth_cpu(mesh_data: &[u8], light_vp: &[[f32; 4]; 4], res: u32, _bias: f32) -> Vec<u8> {
    ...
    for tri in 0..tri_count {
        ...
        for v in 0..3 {
            let w = clip[v][3];
            if w.abs() < 0.001 { valid = false; break; }
        ...
        let r = res as f32;
        let mut min_x = r; let mut max_x = 0.0f32;
        let mut min_y = r; let mut max_y = 0.0f32;
        for v in 0..3 {
        ...
        let screen = |v: usize| -> [f32; 2] {
            [(ndc[v][0] * 0.5 + 0.5) * r, (ndc[v][1] * 0.5 + 0.5) * r]
        };
        let s0 = screen(0); let s1 = screen(1); let s2 = screen(2);
...
fn dot_3(a: [f32;3], b: [f32;3]) -> f32 { a[0]*b[0]+a[1]*b[1]+a[2]*b[2] }

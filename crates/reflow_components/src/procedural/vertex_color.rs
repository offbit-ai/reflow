...
pub async fn vertex_color_actor(ctx: ActorContext) -> Result<HashMap<String, Message>, Error> {
    ...
    if let Some(Message::Bytes(b)) = payload.get("mesh") {
        use base64::Engine;
        ctx.pool_upsert("_cache", "mesh_b64", serde_json::Value::String(
            base64::engine::general_purpose::STANDARD.encode(&**b)));
    }
    if let Some(Message::Bytes(b)) = payload.get("uv") {
        use base64::Engine;
        ctx.pool_upsert("_cache", "uv_b64", serde_json::Value::String(
            base64::engine::general_purpose::STANDARD.encode(&**b)));
    }
    if let Some(Message::Bytes(b)) = payload.get("noise") {
        use base64::Engine;
        ctx.pool_upsert("_cache", "noise_b64", serde_json::Value::String(
            base64::engine::general_purpose::STANDARD.encode(&**b)));
    }
    ...
    let noise_bytes = cache.get("noise_b64").and_then(|v| v.as_str())
        .map(|s| { use base64::Engine; base64::engine::general_purpose::STANDARD.decode(s).unwrap_or_default() });

    let color1 = config.get("color1").and_then(|v| v.as_array())
        .map(|a| [
    ...
    let color2 = config.get("color2").and_then(|v| v.as_array())
        .map(|a| [
    ...
    let noise_scale = config.get("noiseScale").and_then(|v| v.as_f64()).unwrap_or(5.0) as f32;
    let noise_width = config.get("noiseWidth").and_then(|v| v.as_u64()).unwrap_or(64) as usize;
    ...
    for i in 0..vertex_count {
        ...
        // Copy pos + normal
        if mesh_off + in_stride > mesh_bytes.len() { break; }
        ...
        // Sample noise at UV position to determine color blend
        let blend = if let Some(ref noise) = noise_bytes {
            // Noise is a 2D grid of f64 values. Map UV to grid position.
            let nx = ((u * noise_scale).abs() as usize) % noise_width;
            let ny = ((v * noise_scale * noise_width as f32).abs() as usize) % noise_width;
            let idx = (ny * noise_width + nx) * 8; // f64 = 8 bytes
            if idx + 8 <= noise.len() {
                let val = f64::from_le_bytes(noise[idx..idx + 8].try_into().unwrap());
                ((val + 1.0) * 0.5).clamp(0.0, 1.0) as f32 // map [-1,1] to [0,1]
            } else {
                0.5
            }
        } else {
            // No noise: use simple UV-based pattern
            let pattern = ((u * noise_scale).sin() * 0.5 + (v * noise_scale * 6.0).sin() * 0.3 + 0.5).clamp(0.0, 1.0);
            pattern
        };

        // Blend colors
        let color = [
            color1[0] * blend + color2[0] * (1.0 - blend),
            color1[1] * blend + color2[1] * (1.0 - blend),
            color1[2] * blend + color2[2] * (1.0 - blend),
        ];
        for f in &color { output.extend_from_slice(&f.to_le_bytes()); }
    }

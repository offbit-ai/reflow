...
pub async fn vertex_color_actor(ctx: ActorContext) -> Result<HashMap<String, Message>, Error> {
    ...
    for i in 0..vertex_count {
        ...
        let diamond = ((u * noise_scale * 8.0 + v * 12.0).sin()
            * (u * noise_scale * 8.0 - v * 12.0).sin() * 0.5 + 0.5).clamp(0.0, 1.0);
        ...
        let color = [
            (back_color[0] * (1.0 - belly_blend * 0.4) + belly_color[0] * belly_blend * 0.4).clamp(0.0, 1.0),
            (back_color[1] * (1.0 - belly_blend * 0.4) + belly_color[1] * belly_blend * 0.4).clamp(0.0, 1.0),
            (back_color[2] * (1.0 - belly_blend * 0.4) + belly_color[2] * belly_blend * 0.4).clamp(0.0, 1.0),
        ];

...
//!
//! Pure DAG actor. Takes time input, outputs the current frame index.
//! No skeleton, no sampler, no bones. Wire directly to billboard atlas.
//!
//! ## Config
//!
//! ```json
...
//!
//! ```text
//! IntervalTrigger → tick → SpriteAnimation → frame → billboard.atlas.frame
//! ```
//!
//! Or driven by external time:
//!
//! ```text
...
pub async fn sprite_animation_actor(
    ...
    let sprite_fps = config
        .get("fps")
        .and_then(|v| v.as_f64())
        .unwrap_or(12.0);
    let start_frame = config
        .get("startFrame")
        .and_then(|v| v.as_u64())
        .unwrap_or(0) as usize;
    let end_frame = config
        .get("endFrame")
        .and_then(|v| v.as_u64())
        .map(|v| v as usize)
    ...
    let seq_idx = ((effective_time / frame_duration) as usize).min(seq_len.saturating_sub(1));

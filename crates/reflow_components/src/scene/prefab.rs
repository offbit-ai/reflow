...
pub async fn prefab_actor(ctx: ActorContext) -> Result<HashMap<String, Message>, Error> {
    ...
    if let Some(path) = db_path {
        ...
        if let Some(Message::Bytes(b)) = payload.get("mesh") {
            let encoded = { use base64::Engine; base64::engine::general_purpose::STANDARD.encode(&**b) };
        ...
        if let Some(Message::Object(obj)) = payload.get("component") {
            ...
            if !comp_name.is_empty() { ctx.pool_upsert("_components", &comp_name, v); }
        }

        // Write config components
        for (key, val) in &config {
            if CONTROL_KEYS.contains(&key.as_str()) || key.starts_with('$') { continue; }
            match val {
        ...
        for (key, val) in ctx.get_pool("_data").into_iter() {
            match key.as_str() {
                ...
                "material" => { db.merge_component_json(&entity_name, "material", val, json!({}))?; }
                "transform" => { db.merge_component_json(&entity_name, "transform", val, json!({}))?; }
                _ => {}
            }
        }

        for (_, comp) in ctx.get_pool("_components").into_iter() {
            if let (Some(cn), Some(cd)) = (comp.get("name").and_then(|v| v.as_str()), comp.get("data")) {
        ...
        out.insert("prefab".to_string(), Message::object(EncodableValue::from(json!({"id": entity_name, "components": components}))));
        out.insert("metadata".to_string(), Message::object(EncodableValue::from(json!({"action": "define", "entity": entity_name, "components": components}))));
        return Ok(out);
    }
    ...
    let material = match payload.get("material") {
        ...
        _ => config.get("material").cloned().unwrap_or(json!({"color": [0.8, 0.8, 0.8]})),
    };
    ...
    let mut out = HashMap::new();
    out.insert("prefab".to_string(), Message::object(EncodableValue::from(prefab)));
    if let Some(mesh) = payload.get("mesh") {
        out.insert("prefab_mesh".to_string(), mesh.clone());
    }
    out.insert("metadata".to_string(), Message::object(EncodableValue::from(json!({"name": entity_name, "meshSize": mesh_size}))));
    Ok(out)
}

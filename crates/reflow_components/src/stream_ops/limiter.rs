//! Brickwall limiter — prevents audio from exceeding a ceiling.
//!
//! Uses DynamicsProcessor with ratio=∞ for hard limiting.
...
pub async fn limiter_actor(context: ActorContext) -> Result<HashMap<String, Message>, Error> {
    ...
    let attack_ms = config
        .get("attackMs")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.1); // very fast attack for limiting
    ...
    spawn_stream_task(async move {
        #[cfg(feature = "av-core")]
        {
            // ratio=∞ makes this a brickwall limiter
            let mut proc = reflow_dsp::envelope::DynamicsProcessor::compressor(
                ceiling_db,
        ...
        {
            let _ = (ceiling_db, attack_ms, release_ms, sample_rate);
            let mut stream = input_rx.into_stream();
            while let Some(frame) = stream.next().await {
                let is_terminal = frame.is_terminal();

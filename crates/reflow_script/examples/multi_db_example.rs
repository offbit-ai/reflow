...
use reflow_network::actor::Actor;
...
async fn main() -> Result<()> {
    ...
    pool_manager.execute_command(users_db_id, create_users, vec![]).await?;
    
    let create_products = "CREATE TABLE products (id INTEGER PRIMARY KEY, name TEXT, price REAL)";
    pool_manager.execute_command(products_db_id, create_products, vec![]).await?;
    ...
    for (name, email) in users {
        ...
        pool_manager.execute_command(users_db_id, insert_user, params).await?;
    }
    ...
    for (name, price) in products {
        ...
        pool_manager.execute_command(products_db_id, insert_product, params).await?;
    }
    ...
    let users_result = users_behavior(users_query, state.clone(), users_outports.clone()).await?;
    ...
    let products_result = products_behavior(products_query, state.clone(), products_outports.clone()).await?;
    ...
    let users_status = pool_manager.get_connection_status(users_db_id).await?;
    println!("Users DB status after reconnection attempt: {:?}", users_status);

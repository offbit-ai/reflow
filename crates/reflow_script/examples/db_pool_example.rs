...
async fn main() -> Result<()> {
    ...
    pool_manager
        .execute_command(connection_id, create_table, vec![])
        .await?;
    ...
    // Insert first user
    let params1 = vec![json!("Alice"), json!("alice@example.com")];
    pool_manager
        .execute_command(connection_id, insert, params1)
        .await?;

    // Insert second user
    let params2 = vec![json!("Bob"), json!("bob@example.com")];
    pool_manager
        .execute_command(connection_id, insert, params2)
        .await?;

    // Query the data
    let query = "SELECT * FROM users";
    let results = pool_manager
        .execute_query(connection_id, query, vec![])
        .await?;

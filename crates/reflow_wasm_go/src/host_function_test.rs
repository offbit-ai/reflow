...
mod host_function_tests {
    ...
    async fn test_go_counter_with_host_functions() {
        ...
        payload.insert(
            "operation".to_string(),
            Message::String("increment".to_string().into()),
        );
        inports.0.send_async(payload).await.unwrap();
        ...
        payload.insert(
            "operation".to_string(),
            Message::String("double".to_string().into()),
        );
        inports.0.send_async(payload).await.unwrap();

        let mut result = timeout(Duration::from_secs(5), outports.1.recv_async())
            .await
            .expect("Timeout waiting for response")
            .expect("Failed to receive response");
        ...
        if let Some(Message::String(_status)) = result.get("status") {
            ...
            result = timeout(Duration::from_secs(5), outports.1.recv_async())
                .await
                .expect("Timeout waiting for response")
                .expect("Failed to receive response");
        ...
        payload.insert(
            "operation".to_string(),
            Message::String("reset".to_string().into()),
        );
        inports.0.send_async(payload).await.unwrap();

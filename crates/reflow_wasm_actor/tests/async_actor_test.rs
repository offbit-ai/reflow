...
mod tests {
    ...
    async fn test_async_actor_sends_multiple_outputs() {
        ...
        let result = timeout(Duration::from_secs(2), outports.1.recv_async())
            .await
        ...
        let progress_updates: Vec<_> = outputs
            .iter()
            .filter(|o| o.contains_key("progress"))
            .collect();
    ...
    async fn test_async_actor_without_progress() {
        ...
        assert!(
            outputs.len() >= 2,
            "Should have at least initial status and result"
        );
        ...
        assert!(
            ...
            "Should not have progress updates when disabled"
        );
    }
}

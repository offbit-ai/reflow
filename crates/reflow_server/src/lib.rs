...
use reflow_network::api_kit::{
    ApiToolGenerator, DefaultAuthManager, DefaultRateLimiter, ServiceRegistry,
};
...
impl AppState {
    pub fn new() -> Self {
        Self {
            network: Arc::new(tokio::sync::Mutex::new(Network::new(
        ...
        for (template_id, actor_name) in template_mappings {
            ...
            if let Some(actor) = reflow_components::get_actor_for_template(&template_id) {
                ...
                if let Err(e) = network.register_actor_arc(&template_id, actor) {
                    warn!(
                        "Failed to register actor for template {}: {}",
                        template_id, e
                    );
                } else {
    ...
    async fn execute_workflow_background(
        ...
        match Self::execute_graph_background(
            &request.graph_json,
            &request.input,
            &event_sender,
            &execution_id,
    ...
    async fn detect_terminal_nodes(
        ...
        for (node_id, _node) in &graph.nodes {
            let has_outputs = graph
                .connections
                .iter()
                .any(|conn| conn.from.node_id == *node_id);
            if !has_outputs {
            ...
            .await
        {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    warn!(
                        "Trace event failed with status: {} for {}",
                        response.status(),
                        trace_url
                    );
                }
        ...
        match request_builder.send().await {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    warn!(
                        "Webhook notification failed with status: {} for {}",
                        response.status(),
                        webhook_url
                    );
                }
    ...
    fn create_network_with_graph_and_actors(
        graph: &Graph,
    ...
    pub fn subscribe_to_workflow(
        &self,
        execution_id: &str,
    ) -> Option<flume::Receiver<WorkflowEvent>> {
        self.executions
            .get(execution_id)
            .and_then(|state| state.event_receiver.clone())
    }
    ...
    pub async fn cancel_workflow(&self, execution_id: &str) -> Result<()> {
        if let Some(mut state) = self.executions.get_mut(execution_id) {
            ...
            if let Some(network_handle) = &state.network_handle {
                ...
                network.shutdown();
                info!(
                    "Cancelled workflow execution: {} (network shutdown)",
                    execution_id
                );
            }
    ...
    pub async fn execute_zeal_workflow(
        &self,
    ...
    match state.register_api_tools().await {
        ...
        Err(e) => {
            ...
            Json(ApiResponse::error(
                "Failed to register API tools".to_string(),
            ))
        }
    }
}
                ...
                match msg_type {
                    "start_workflow" => {
                        if let Some(request_data) = json.get("data") {
                            match serde_json::from_value::<WorkflowExecutionRequest>(
                                request_data.clone(),
                            ) {
                                Ok(request) => match state.start_workflow(request).await {
                                    Ok(execution_id) => {
                    ...
                    "subscribe_workflow" => {
                        if let Some(execution_id) =
                            json.get("execution_id").and_then(|id| id.as_str())
                        {
                            ...
                            if let Some(exec_state) = state.executions.get(execution_id) {
                                ...
                                if let Some(network_handle) = exec_state.network_handle.clone() {
                                    ...
                                    tokio::spawn(async move {
                                        info!(
                                            "Starting direct network subscription for execution: {}",
                                            exec_id
                                        );
                                        ...
                                        info!(
                                            "Network subscription ended for execution: {}",
                                            exec_id
                                        );
                                    });
                                ...
                                } else if let Some(event_receiver) =
                                    exec_state.event_receiver.clone()
                                {
                                    ...
                                    tokio::spawn(async move {
                                        info!(
                                            "Starting workflow event subscription for execution: {}",
                                            exec_id
                                        );
                                        ...
                                        info!(
                                            "Workflow event subscription ended for execution: {}",
                                            exec_id
                                        );
                                    });
...
mod tests {
    ...
    fn create_test_zeal_workflow() -> ZealWorkflow {
        ZealWorkflow {
            ...
            graphs: vec![zeal_converter::ZealGraph {
                ...
                is_main: true,
                nodes: vec![zeal_converter::ZealNode {
    ...
    async fn test_register_zeal_actors() {
        ...
        assert!(
            result.is_ok(),
            "Failed to register Zeal actors: {:?}",
            result.err()
        );
        ...
        assert!(
            registered_count > 0,
            "Should have registered at least some actors"
        );
    ...
    async fn test_execute_zeal_workflow_http() {
        ...
        assert!(
            result.is_ok(),
            "Zeal HTTP workflow execution should succeed: {:?}",
            result.err()
        );

        let execution_id = result.unwrap();
        assert!(
            execution_id.starts_with("zeal_exec_"),
            "Execution ID should have correct prefix"
        );

        // Verify execution is tracked
        assert!(
            state.executions.contains_key(&execution_id),
            "Execution should be tracked in state"
        );
    ...
    async fn test_execute_zeal_workflow_postgresql() {
        ...
        assert!(
            result.is_ok(),
            "Zeal PostgreSQL workflow execution should succeed: {:?}",
            result.err()
        );
    ...
    async fn test_workflow_execution_request() {
        ...
        assert!(
            result.is_ok(),
            "Workflow execution should start successfully: {:?}",
            result.err()
        );
    ...
    async fn test_cancel_workflow() {
        ...
        assert!(
            cancel_result.is_ok(),
            "Workflow cancellation should succeed: {:?}",
            cancel_result.err()
        );
    }
    ...
    async fn test_cancel_nonexistent_workflow() {
        ...
        assert!(
            ...
            "Should fail to cancel nonexistent workflow"
        );
    ...
    async fn test_get_execution_status_nonexistent() {
        ...
        assert!(
            ...
            "Should fail to get status of nonexistent execution"
        );
    }
    ...
    async fn test_zeal_workflow_conversion() {
        ...
        assert!(
            result.is_ok(),
            "Zeal workflow conversion should succeed: {:?}",
            result.err()
        );
        ...
        assert!(
            graph_export.processes.contains_key("http_node"),
            "Should contain the http node"
        );
    }
    ...
    async fn test_concurrent_workflow_executions() {
        ...
        for i in 0..3 {
            ...
            let handle = tokio::spawn(async move {
                state_clone
                    .execute_zeal_workflow(workflow, json!({"index": i}))
                    .await
            });
            handles.push(handle);
        }
    ...
    async fn test_integration_start_server_and_execute_workflow() {
        ...
        assert_eq!(
            response.status(),
            200,
            "Execute endpoint should return 200 OK"
        );
        ...
        assert!(
            get_response.is_ok(),
            "Get execution endpoint should be reachable"
        );
        ...
        assert!(
            ...
            "Cancel endpoint should be reachable"
        );
    ...
    async fn test_integration_zeal_workflow_endpoint() {
        ...
        assert!(
            response.is_ok(),
            "Zeal execute endpoint should be reachable"
        );
        let response = response.unwrap();
        ...
        assert!(
            ...
            "Zeal execute should return 200 OK or 400 Bad Request (for invalid format), got: {}",
            status
        );
        ...
        assert!(
            get_response.is_ok(),
            "Should be able to call get execution endpoint"
        );
        let get_response = get_response.unwrap();
    ...
    async fn test_integration_nonexistent_endpoints() {
        ...
        let cancel_response = client
            .post(&format!(
                "{}/workflows/nonexistent_execution/cancel",
                base_url
            ))
            .send()
            .await;
        ...
        assert_eq!(
            ...
            404,
            "Should return 404 for nonexistent execution cancel"
        );
    ...
    async fn test_integration_server_health_and_basic_endpoints() {
        ...
        assert!(
            ws_response.is_ok(),
            "WebSocket endpoint should be reachable"
        );

...
pub fn build_stream_actor_templates(
    ...
    let mut templates = vec![
        ...
        tpl("tpl_sdf_torus", "SDF Torus", "Torus primitive",
            ...
            vec![inport("trigger", "Trigger", "any"), outport("sdf", "SDF", "object")],
            props(vec![
                ("majorRadius", num_prop("Major Radius", 1.0, 0.1, 50.0, "Ring radius")),
        ...
        tpl("tpl_sdf_cylinder", "SDF Cylinder", "Cylinder primitive",
            ...
            vec![inport("trigger", "Trigger", "any"), outport("sdf", "SDF", "object")],
            props(vec![
                ("radius", num_prop("Radius", 0.5, 0.01, 50.0, "Cylinder radius")),
                ("height", num_prop("Height", 1.0, 0.01, 100.0, "Half-height")),
            ]), v, c),
        ...
        tpl("tpl_sdf_repeat", "Repeat", "Finite repetition",
            ...
            vec![inport("sdf", "SDF", "object"), outport("sdf", "SDF", "object")],
            props(vec![
                ("spacingX", num_prop("Spacing X", 2.0, 0.1, 50.0, "X spacing")),
                ("spacingY", num_prop("Spacing Y", 2.0, 0.1, 50.0, "Y spacing")),
                ("spacingZ", num_prop("Spacing Z", 2.0, 0.1, 50.0, "Z spacing")),
                ("countX", num_prop("Count X", 3.0, 1.0, 20.0, "X repetitions")),
                ("countY", num_prop("Count Y", 1.0, 1.0, 20.0, "Y repetitions")),
        ...
        tpl("tpl_sdf_displace", "Displace", "Noise displacement",
            ...
            vec![inport("sdf", "SDF", "object"), outport("sdf", "SDF", "object")],
            props(vec![
                ("frequency", num_prop("Frequency", 3.0, 0.1, 50.0, "Noise frequency")),
                ("amplitude", num_prop("Amplitude", 0.1, 0.0, 2.0, "Displacement amount")),
        ...
        tpl("tpl_sdf_render", "SDF Render", "GPU ray march to image",
            ...
            props(vec![
                ("width", num_prop("Width", 512.0, 64.0, 4096.0, "Output width")),
                ("height", num_prop("Height", 512.0, 64.0, 4096.0, "Output height")),
                ("maxSteps", num_prop("Max Steps", 128.0, 16.0, 512.0, "Ray march iterations")),
                ("fov", num_prop("FOV", 45.0, 10.0, 120.0, "Field of view (degrees)")),
        ...
        tpl("tpl_sdf_marching_cubes", "Marching Cubes", "SDF → triangle mesh (GPU)",
            ...
            props(vec![
                ("resolution", num_prop("Resolution", 64.0, 8.0, 256.0, "Grid density")),
        ...
        tpl("tpl_mesh_to_sdf", "Mesh to SDF", "Triangle mesh → distance field (GPU)",
            ...
            props(vec![
                ("resolution", num_prop("Resolution", 32.0, 8.0, 128.0, "Volume grid density")),
        ...
        tpl("tpl_file_load", "File Load", "Read file to bytes",
            "tools-utilities", "files", "Reads a file from disk into Message::Bytes with MIME detection.",
            "upload", "slate-500",
            vec![inport("trigger", "Trigger", "any"), outport("output", "Data", "bytes"), outport("metadata", "Metadata", "object")],
            props(vec![("path", str_prop("Path", "", "File path to read"))]),
            v, c),
        tpl("tpl_file_save", "File Save", "Write bytes to file",
            "tools-utilities", "files", "Writes Message::Bytes to a file on disk.",
            "download", "slate-500",
            vec![inport("input", "Data", "bytes"), outport("path", "Path", "string"), outport("metadata", "Metadata", "object")],
            props(vec![

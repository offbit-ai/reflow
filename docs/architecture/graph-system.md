...
The `SubgraphActor` wraps an entire inner `Network` as a single `Actor`, allowing a graph to be embedded inside another graph as a composable unit. This is the foundation of hierarchical workflow composition.
...
Bridge actors are registered inside the inner network with generated IDs (`__outport_bridge_{ext_port}`) and connected to source actors via standard Connectors.
...
1. `create_process()` starts the inner network and begins the inbound routing loop
2. The inner network runs its actors concurrently
3. `shutdown()` signals the routing loop to stop and shuts down the inner network
...
- [Distributed Networks](./distributed-networks.md) - Cross-network execution and PeerMesh
